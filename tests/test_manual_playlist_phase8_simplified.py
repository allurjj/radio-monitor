...
# -*- coding: utf-8 -*-
"""
Phase 8 Testing Script - Manual Playlist Builder (Simplified)
...
class Phase8Tester:
    ...
    def test_8_3_edge_cases(self):
        ...
        try:
            ...
            special_name = f"Test {timestamp} 's Playlist & More! - SpecialChars: @#$%"
            ...
            start_time = time.time()
            timestamp = datetime.now().strftime("%H%M%S")
            long_name = f"A{timestamp}" + "A" * (200 - len(timestamp) - 1)  # Exactly 200 chars
            ...
            # Test 3.4: Duplicate playlist names (should NOT allow - UNIQUE constraint)
            start_time = time.time()
            timestamp = datetime.now().strftime("%H%M%S")
            duplicate_name = f"Duplicate Test {timestamp}"

            pid1 = create_manual_playlist(cursor, db.conn, name=duplicate_name)

            # Try to create duplicate - should fail
            try:
                pid2 = create_manual_playlist(cursor, db.conn, name=duplicate_name)
                # If we get here, duplicates are allowed (BAD)
                db.conn.commit()
                duration = (time.time() - start_time) * 1000
                self.log_test("Duplicate Playlist Names", False,
                            "Duplicates should NOT be allowed (UNIQUE constraint)", duration)
            except Exception as e:
                # Expected - duplicates should fail
                duration = (time.time() - start_time) * 1000
                if "UNIQUE constraint failed" in str(e):
                    self.log_test("Duplicate Playlist Names", True,
                                "Correctly rejected (UNIQUE constraint)", duration)
                else:
                    self.log_test("Duplicate Playlist Names", False,
                                f"Wrong error: {e}", duration)
            ...
            """)
            cursor.execute("DELETE FROM manual_playlist_songs WHERE manual_playlist_id NOT IN (SELECT id FROM manual_playlists)")
            db.conn.commit()
    ...
    def test_8_4_database_operations(self):
        ...
        try:
            ...
            playlist_id = create_manual_playlist(
                cursor,
                db.conn,
                name=f"Test CRUD {datetime.now().strftime('%H%M%S')}",
                plex_playlist_name=f"Plex CRUD {datetime.now().strftime('%H%M%S')}"
            )

            duration = (time.time() - start_time) * 1000

            self.log_test("Create Playlist", True,
                        f"Created playlist {playlist_id}", duration)
            ...
            if playlist and playlist['name'].startswith("Test CRUD"):
            ...
            if playlist and playlist['name'].startswith("Updated CRUD"):
