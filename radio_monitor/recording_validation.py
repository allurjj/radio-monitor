...
def validate_recording_by_mbid(artist_mbid: str, song_title: str, clean_title: bool = True) -> Tuple[bool, str]:
    """Validate recording using artist MBID (most precise)

    Args:
        artist_mbid: MusicBrainz artist ID
        song_title: Song title to validate
        clean_title: Whether to clean title before query (default: True)
    ...
    for recording in recordings:
        if is_recording_match(recording, query_title, ""):  # Artist implied by MBID
...
def validate_recording_by_text(artist_name: str, song_title: str, clean_title: bool = True) -> Tuple[bool, str]:
    ...
    # Check for exact match
    for recording in recordings:
        if is_recording_match(recording, query_title, artist_name):
...
def validate_recording_with_fallback(artist_mbid: Optional[str], artist_name: str,
    ...
    if artist_mbid and not artist_mbid.startswith('PENDING-'):
        found, method = validate_recording_by_mbid(artist_mbid, song_title, clean_title=True)
        if found:
            return True, 'mbid'
    ...
    found, method = validate_recording_by_text(artist_name, song_title, clean_title=True)
    if found:
