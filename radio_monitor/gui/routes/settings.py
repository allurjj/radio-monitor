...
def api_validate_spotiflac_path():
    """Validate SpotiFLAC download path is accessible and writable"""
    ...
    try:
        ...
        result = service.validate_lidarr_path(path)

        if result['valid']:
            return jsonify({
                'valid': True,
                'path': result['path'],
                'message': f'Path is valid and writable: {result["path"]}'
            })
        else:
            return jsonify({
                'valid': False,
        ...
        logger.error(f"Error validating SpotiFLAC path: {e}", exc_info=True)
        return jsonify({
            'valid': False,
            'error': str(e)
        }), 500
...
@settings_bp.route('/api/spotiflac/check-api-health', methods=['GET'])
@requires_auth
def api_check_spotiflac_api_health():
    ...
    Returns JSON:
        {
            ...
            "services": {
                "songlink": {"healthy": true/false, "message": "..."},
                "spotidownloader": {"healthy": true/false, "message": "..."},
                "qobuz": {"healthy": true/false, "message": "..."}
            }
        }
    """
    try:
        ...
        health = service._check_external_api_health()
        ...
        try:
            import requests
            resp = requests.get("https://song.link", timeout=5)
            services_status['songlink'] = {
        ...
        # Check SpotiDownloader
        try:
            import requests
            resp = requests.get("https://spdl.afkarxyz.fun/token", timeout=5)
            services_status['spotidownloader'] = {
                'healthy': resp.status_code == 200,
                'message': 'OK' if resp.status_code == 200 else f'HTTP {resp.status_code}'
            }
        except Exception as e:
        ...
        try:
            import requests
            resp = requests.get("https://qbz.afkarxyz.fun/api/track/1", timeout=5)
            services_status['qobuz'] = {
                'healthy': resp.status_code in [200, 404],  # 404 is OK (track doesn't exist)
                'message': 'OK' if resp.status_code in [200, 404] else f'HTTP {resp.status_code}'
            }
        except Exception as e:
            services_status['qobuz'] = {
                'healthy': False,
        ...
        logger.error(f"Error checking SpotiFLAC API health: {e}", exc_info=True)
        return jsonify({
            'all_healthy': False,
